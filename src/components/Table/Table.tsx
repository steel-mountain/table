...
// const colorHoverRow = "#eceff6";
// const timeForTrottle = 300;

export const CustomTable: FC<CustomTableProps> = memo(
  ({ columns, data, tableProps, fetchMoreData, isLoading, isFetching, isFlattenArray }) => {
    const {
      wrapperStyle,
      headerStyle,
      bodyStyle,
    ...
    const childSecondTableRef = useRef<HTMLDivElement | null>(null);
    ...
    const rowVirtualizer = useVirtualizer({
      ...
      estimateSize: (i) => {
        ...
        if (expandedIndexes.includes(dataTable[i].id)) {
          return rowHeight + dataTable[i].children?.length * rowHeight;
        }
        return rowHeight;
      },
      overscan: 3,
